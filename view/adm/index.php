<?php
    ...
    $resultUser = showData( "SELECT * FROM user" );

    if( isset($_POST["search"]) ) {
        $resultUser = searchDataUser($_POST["keyword"]);
    }
?>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div class="border-end bg-white" id="sidebar-wrapper">
            ...
            <div class="list-group list-group-flush">
                <a class="list-group-item list-group-item-action list-group-item-light p-3 active" href="index.php">User</a>
                            ...
                            <table class="table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="text-wrap">USERNAME</th>
                                        <th scope="col" class="text-wrap">EMAIL</th>
                                        <th scope="col" class="text-wrap">CREATED</th>
                                        <th scope="col" class="text-wrap">UPDATED</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $i = 1; ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="./update.php?id=<?= $row['id'] ?>" class="btn btn-dark btn-sm"><span class="iconify" data-icon="ph:note-pencil" data-width="20" data-height="20"></span></a>
                                            <a href="./delete.php?id=<?= $row["id"]; ?>" onclick="return confirm('Anda yakin?')" class="btn btn-danger btn-sm"><span class="iconify" data-icon="ph:trash" data-width="20" data-height="20"></span></a>
                                        </td>
                                        <td class="text-wrap text-center"><?= $row["role_id"]; ?></td>
                                        <td class="text-wrap text-sm-start"><?= $row["username"]; ?></td>
                                        <td class="text-wrap text-sm-start"><?= $row["email"]; ?></td>
                                        <td class="text-wrap text-sm-start"><?= date('d F Y', strtotime($row['created_at'])) ?></td>
                                        <td class="text-wrap text-sm-start"><?= $row["updated_at"]; ?></td>
                                    </tr>
                                    <?php $i++; ?>
                                    <?php endforeach; ?>
                                </tbody>
